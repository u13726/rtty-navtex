...
#define SAMRAT (11025 * 3)
#define SAMPLE_BUFFER 8192
AudioInfo info(SAMRAT, 1, 16);
int nPages = 22;
AudioBoardStream kit(AudioKitEs8388V1);  // Access I2S as stream
VolumeMeter vols(kit);
FilteredStream<int16_t, float> filtered(vols, info.channels);
GoertzelStream goertzel(filtered);
StreamCopy copier(kit, goertzel);
float MARK = 2125;
float SPACE = 2295;
float cf;
float mgb = 10;
...
void setBS(short s) {
  ...
  if (s != pbs || s == 0) {
    if (s == 0) s = 1;
    gcfg.block_size = (((1000 / baud_tab[bidx]) * (SAMRAT / 10))) / (overs * s * 100) + 0;
...
void handle(char ch) {
  ...
  if (ch == 'i') {
    ...
    printf("baud:%d,mark:%.0f,space:%.0f,shift:%d, %s ,bsize:%d overs:%d SYN  %f filt :%srs\n",
           baud_tab[bidx], MARK, SPACE, shift_tab[sidx], hsb ? "HSB" : "No HSB", gcfg.block_size,
...
void setfil() {
  if (filuse) {  //filtered.setFilter(0, new MyFilter(filact?0.024:0.0,filact?0.5:0.0,MARK,SPACE));
    if (filact) {
      ...
      F1 = F2 = F3 = NULL;
      filtered.setFilter(0, new FilterChain<float, 3 >({ F1 = new BandPassFilter<float>(cf, SAMRAT, filact ? 4000.0 / cf : 1000.00 / cf),
                                                         F2 = new BandPassFilter<float>(SPACE, SAMRAT, filact ? 6000 / cf : 1000.00 / cf),
                                                         F3 = new BandPassFilter<float>(MARK, SAMRAT, filact ? 6000 / cf : 1000.00 / cf) }));  //, new BiQuadDF2<float>(bb[2],aa[2])}));
    } else
      filtered.setFilter(0, new LowPassFilter<float>(4000.0, SAMRAT, 1.0));
...
void rstfre(float f1, float f2) {
  ...
  //swm=f1-75;sws=f2+75;
  putmar(f1, f2);
  cf = MARK + (SPACE - MARK) / 2;
}
bool prim = true;
unsigned long brktim;
void loop() {
  int b = copier.copyMs(nPages * 1, info);  //copier.copyMs(7*1000/100 , info);// / baud_tab[bidx], info);
  if (b == 0) Serial.print("@");
  {
    static unsigned long vol;
    static short nvol = 0;
    vol += vols.volume() / 100;
    nvol++;
    if (nvol == 1000) {
      Serial.printf("vol: %d heap : %d free %d \n", vol / nvol, ESP.getHeapSize(), ESP.getFreeHeap());
      vol = 0;
      nvol = 0;
    }
  }
